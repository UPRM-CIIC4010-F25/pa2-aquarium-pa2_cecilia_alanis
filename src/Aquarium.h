...
class AquariumLevel : public GameLevel {
    public:
        ...
        std::vector<AquariumCreatureType> RepopulateTemplate() {
...
class DoryFish : public NPCreature {
public:
    DoryFish(float x, float y, int speed, std::shared_ptr<GameSprite> sprite);
    void move() override;
    void draw() const override;

private:
    int m_movementTimer = 0; // Timer for DoryFish's forgetful behavior
};
...
class AquariumSpriteManager {
    ...
    private:
        ...
        std::shared_ptr<GameSprite> m_dory_fish;
};
...
class Level_4 : public AquariumLevel {
    public:
        Level_4(int levelNumber, int targetScore): AquariumLevel(levelNumber, targetScore) {
            ...
            this->m_levelPopulation.push_back(std::make_shared<AquariumLevelPopulationNode>(AquariumCreatureType::NemoFish, 7));
            this->m_levelPopulation.push_back(std::make_shared<AquariumLevelPopulationNode>(AquariumCreatureType::DoryFish, 5));
        };
        std::vector<AquariumCreatureType> Repopulate() override;
};
